...
class Client(threading.Thread):
  ...
  def handshake(self, debug = False):
    ...
    # Certificado e chave pública do cliente
    ...
    self.client_public_key =  self.client_certificate.public_key() # Retira chave pública do certificado do cliente.
  ...
  # serve para provar/autenticar que é o servidor que está a mandar a mensagem(não repúdio), uma vez
  # que só o servidor tem esta chave privada
  # a mensagem ao ser g^y,g^x tem como objetivo manter a integridade destes, já que
  # um atacante no meio da conexão poderia alterar g^x/g^y, após assinatura
  # cliente pode verificar a assinatura garantindo o não repúdio e integridade da mensagem.
  ...
  # Verifica se a assinatura do cliente foi mesmo realizada por este, já que ao verificarmos com a chave pública
  # com o certificado temos uma maior segurança contra ataques man-in-the-middle,
  # com a verificação garantimos que a mensagem não foi modificada e foi gerada com a chave privada do cliente,
  # dado que só a chave pública do cliente a pode verificar.
  ...
  # Verifica se o dados da entidade responsável pelo certificado (CA)
  # são os mesmos que estão no certificado
  # e verifica se este certificado foi assinado pela CA, de modo a verificar se é confiável,
  # com intuito de impedir que alguém por exemplo num ataque man-in-the-middle
  # forneça um certificado que realmente não pertença a quem esteja a enviar informação.
